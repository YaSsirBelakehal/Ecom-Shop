...
const Sidebar = () => {
  ...
  const menuItems = [
    { to: "/admin/add", label:"add a product", Icon:PlusCircle},
    { to: "/admin/list", label:"product list", Icon:List},
  ...
  return (
    <>
      ...
      fixed top-4 left-4 z-50 bg-indigo-600 p-3 rounded-xl text-white
      shadow-lg transition-transform duration-300 hover:scale-105 focus:outline-none">
        {isOpen ? <X className="w-5 h-5 cursor-pointer"/>
        : <Menu className="w-5 h-5 cursor-pointer"/>}

      </button>

      <aside className={`fixed top-0 left-0 h-full w-64 bg-linear-to-b
        ...
        ${isOpen ? "translate-x-0" : "-translate-x-full"} md:translate-x-0`}>
          <div className="flex flex-col h-full justify-between py-10 px-6">
            ...
            <button onClick={handleLogout} className="flex items-center gap-2 justify-center mt-6
            w-full px-4 py-3 bg-red-600 rounded-xl hover:bg-red-700">
